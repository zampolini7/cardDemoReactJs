import { React, useContext } from "react";
import { CardList } from "../../components/cardlist";
import { Card } from "../../components/card";
import { CardContext }  from "../../components/CardContext";

// const Defaultcards = [{ "id": 1, 
//                 "name": "Nicolas zampolini",
//                 "numberOfCreditCard": 123456789101112,
//                 "until": "12/24", 
//                 "creditCard": "Naranja",
//                 "img": "https://toppng.com/uploads/preview/logo-tarjeta-naranja-115501048913p0sorv6yj.png",
//                 "kindOfCreditCard": "Credit"
// },
//                 { "id": 2, 
//                 "name": "Nicolas zampolini",
//                 "numberOfCreditCard": 123456789101112,
//                 "until": "12/24", 
//                 "creditCard": "Visa",
//                 "img": "data:image/jpeg;base64,",
//                 "kindOfCreditCard": "Credit"
// },
//                 { "id": 3, 
//                 "name": "Nicolas zampolini",
//                 "numberOfCreditCard": 123456789101112,
//                 "until": "12/24", 
//                 "creditCard": "Mastercard",
//                 "img": "https://upload.wikimedia.org/wikipedia/commons/0/04/Mastercard-logo.png",
//                 "kindOfCreditCard": "Credit"
// }

// ]

const CardListContainer = () =>{

  const { cards } = useContext(CardContext) 

 
  
  return(
      
      cards.length > 0? 
      <CardList>
        {
        cards.map((cards, i) =>(            
              <Card 
              key={i}
              id={cards.id}
              name={cards.name}
              numberOfCreditCard={cards.numberOfCreditCard}
              until= {cards.until}
              creditCard= {cards.creditCard}
              img={cards.img}
              kindOfCreditCard={cards.kindOfCreditCard}
              // deleteFunction={()=> deleteFunction(cards.id)}
              keyOfSecurity = {cards.keyOfSecurity}
              
              
              />
                
         ))
        }
      </CardList>
      : <h2> No tienes tarjetas aún, agrega una para simplificar tus pagos</h2>
    
  )
}

export { CardListContainer }